...
export default async function ProgrammationPage() {
  const [clients, progs] = await Promise.all([
    ...
    (async () => { try { return await (prisma as any).clientProgramming.findMany({ select: { clientId: true, accessCode: true } }) } catch { return [] } })(),
  ])
  const hasCode = new Set((progs as any[]).filter(p => p.accessCode).map(p => p.clientId))
