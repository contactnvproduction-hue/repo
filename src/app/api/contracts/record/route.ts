...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { retainer, invoices } = await materializeContract({ clientId, monthlyAmount, durationMonths })
    return NextResponse.json({ ok: true, retainerId: retainer?.id ?? null, invoicesCreated: invoices.length })
  } catch (e) {
