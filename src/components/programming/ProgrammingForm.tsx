...
import {
  ...
  Image as ImageIcon, Calendar, Save, Video, ChevronDown,
} from 'lucide-react'
...
const channelIcon = (c: string) => {
  ...
  if (k.includes('linkedin')) return <Globe size={14} className="text-blue-400" />
  if (k.includes('facebook')) return <Globe size={14} className="text-blue-500" />
    ...
    <div className="min-h-screen bg-nv-black text-white">
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        <div className="text-center">
          ...
          <p className="text-sm text-nv-text-muted mt-1">Renseigne tes paramètres et tes vidéos à venir. Rien n&apos;est obligatoire.</p>
        </div>
        ...
        {clientId && !loading && (
          <>
            ...
            <div className="bg-nv-card border border-nv-border rounded-2xl p-4 space-y-4">
              <h2 className="text-sm font-semibold text-white flex items-center gap-2"><Save size={15} className="text-primary" /> Mes paramètres par défaut</h2>
              ...
              <div>
                ...
                <div className="flex items-start gap-1.5 text-[11px] text-emerald-300/90 bg-emerald-500/8 border border-emerald-500/25 rounded-lg px-2.5 py-1.5 mb-2">
                  <Lock size={12} className="mt-0.5 shrink-0" /> Confidentialité totale : ces informations restent strictement privées et ne servent qu&apos;à publier ton contenu.
                </div>
              ...
              {/* Bio */}
              <div>
                <label className={label}>Bio (réutilisée par défaut sur chaque vidéo)</label>
                ...
                <label className="flex items-center gap-2 mt-2 text-xs text-nv-text cursor-pointer select-none">
                  <input type="checkbox" checked={prog.bioPerVideo} onChange={e => setProg({ ...prog, bioPerVideo: e.target.checked })} className="w-4 h-4 accent-[#e8b84b]" />
                  Bio différenciante — je préfère l&apos;envoyer à chaque vidéo
                </label>
              </div>
              ...
              {/* Mentions YT par défaut */}
              <div>
                <label className={label}>Mentions YouTube par défaut (fiches, écrans de fin…)</label>
                <input className={inp} placeholder="ex : écran de fin vers dernière vidéo, fiche produit…" value={prog.defaultMentions ?? ''} onChange={e => setProg({ ...prog, defaultMentions: e.target.value })} />
              </div>
              ...
              <button onClick={saveDefaults} disabled={savingDefaults} className="w-full flex items-center justify-center gap-1.5 py-2.5 bg-primary text-nv-black rounded-lg font-medium disabled:opacity-60">
                {savingDefaults ? <Loader2 size={15} className="animate-spin" /> : <Check size={15} />} Enregistrer mes paramètres
              </button>
            </div>
            ...
            <div className="bg-nv-card border border-nv-border rounded-2xl p-4 space-y-3">
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-semibold text-white flex items-center gap-2"><PlayCircle size={15} className="text-red-400" /> Mes vidéos à venir</h2>
              ...
              {videos.length === 0 ? (
                <p className="text-xs text-nv-text-faint text-center py-6 border border-dashed border-nv-border rounded-xl">Aucune vidéo programmée.</p>
              ) : (
                <div className="space-y-2">
                  {videos.map(v => (
            ...
            {selected && <p className="text-center text-[11px] text-nv-text-faint">Tes infos sont visibles par l&apos;équipe New Vision sur ta fiche.</p>}
          </>
        )}
      </div>
    </div>
...
function VideoForm({ clientId, bioPerVideo, defaultMentions, onClose, onAdded }: {
  ...
  const [f, setF] = useState({ platform: 'YOUTUBE', title: '', description: '', bio: '', scheduledAt: '', mentions: defaultMentions })
  const [titleVariants, setTitleVariants] = useState<string[]>([])
  const [thumbnailUrl, setThumbnailUrl] = useState('')
  const [thumbVariants, setThumbVariants] = useState<string[]>([])
  const [saving, setSaving] = useState(false)

  const readImage = (file: File | null, cb: (dataUrl: string) => void) => {
    if (!file) return
  ...
  const save = async () => {
    ...
    try {
      const res = await fetch('/api/programming/video', {
        ...
        body: JSON.stringify({ clientId, ...f, titleVariants, thumbnailUrl, thumbnailVariants: thumbVariants, scheduledAt: f.scheduledAt || null }),
      })
      if (!res.ok) throw new Error()
  ...
  return (
    <div className="border border-primary/30 rounded-xl p-3 space-y-3 bg-nv-dark">
      ...
      <div>
        <label className={label}>Miniature (optionnel — parfois fournie par l&apos;équipe)</label>
        <div className="flex items-center gap-2">
          {thumbnailUrl ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={thumbnailUrl} alt="" className="w-24 h-14 object-cover rounded-lg" />
          ) : (
            <div className="w-24 h-14 rounded-lg bg-nv-card border border-nv-border flex items-center justify-center"><ImageIcon size={16} className="text-nv-text-faint" /></div>
          )}
          <label className="text-xs px-3 py-2 rounded-lg border border-nv-border text-nv-text-muted hover:text-white cursor-pointer transition-colors">
            Uploader
            <input type="file" accept="image/*" className="hidden" onChange={e => readImage(e.target.files?.[0] ?? null, setThumbnailUrl)} />
          </label>
        ...
        <div className="mt-1.5 space-y-1.5">
          ...
          <label className="text-[11px] text-primary flex items-center gap-1 cursor-pointer w-fit">
            ...
            <input type="file" accept="image/*" className="hidden" onChange={e => readImage(e.target.files?.[0] ?? null, url => setThumbVariants(v => [...v, url]))} />
          </label>
        </div>
      </div>
      ...
      {bioPerVideo && (
        <div>
          <label className={label}>Bio spécifique à cette vidéo</label>
          <textarea className={`${inp} resize-none`} rows={2} placeholder="Bio différenciante…" value={f.bio} onChange={e => setF({ ...f, bio: e.target.value })} />
        </div>
      )}

      <div>
        <label className={label}>Mentions spécifiques (fiches, écrans de fin…)</label>
        <input className={inp} placeholder="ex : fiche vers site, écran de fin…" value={f.mentions} onChange={e => setF({ ...f, mentions: e.target.value })} />
      </div>
      ...
      <div className="flex justify-end gap-2">
        ...
        <button onClick={save} disabled={saving} className="px-4 py-2 text-xs bg-primary text-nv-black rounded-lg font-medium flex items-center gap-1 disabled:opacity-60">{saving ? <Loader2 size={13} className="animate-spin" /> : <Check size={13} />} Ajouter la vidéo</button>
      </div>
    </div>
  )
}
