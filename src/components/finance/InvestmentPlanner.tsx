...
import { Plus, X, Check, Trash2, Rocket, Target, PiggyBank, TrendingUp, TrendingDown, Wallet } from 'lucide-react'
...
export function InvestmentPlanner({ initial, poles, resultNetYear, monthlyNet = [], forecastByMonth = {} }: {
  ...
  const plannedOf = (key: string) => items.filter(i => i.month === key).reduce((s, i) => s + i.amount, 0)
  ...
  const add = async (month: string, data: { label: string; pole: string; amount: string; notes: string }) => {
    ...
    setItems(list => [...list, inv]); toast.success('Investissement planifié')
  }
    ...
    <div className="space-y-4">
      ...
      <div className="bg-nv-card border border-nv-border rounded-2xl p-4">
        <div className="flex items-center justify-between flex-wrap gap-3 mb-3">
          ...
          <div className="flex items-center gap-4 text-xs">
            ...
            <span className="text-nv-text-muted flex items-center gap-1"><PiggyBank size={13} className="text-emerald-400" /> Épargne cumulée : <span className="font-bold text-emerald-400 tabular-nums">{eur(totalSaved)}</span></span>
          </div>
        </div>
      ...
      {openRow && (
        <MonthModal
          ...
          invests={items.filter(i => i.month === openRow.key)}
...
function MonthModal({ row, fc, poles, alloc, invests, envelopePct, onClose, onAdd, onToggle, onRemove, onSetPolePct }: {
  ...
  onClose: () => void; onAdd: (d: { label: string; pole: string; amount: string; notes: string }) => void
            ...
            <div className="space-y-1.5 mt-1.5">
              {invests.length === 0 ? <p className="text-xs text-nv-text-faint">Aucun investissement — l&apos;enveloppe part en épargne.</p> : invests.map(inv => (
                <div key={inv.id} className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg border text-xs ${inv.done ? 'border-emerald-500/25 bg-emerald-500/5' : 'border-nv-border bg-nv-card'}`}>
                  ...
                  <div className="min-w-0 flex-1"><p className={`truncate ${inv.done ? 'text-nv-text-muted line-through' : 'text-nv-text'}`}>{inv.label}</p>{inv.pole && <p className="text-[9px] text-nv-text-faint truncate">{inv.pole}</p>}</div>
...
function AddInvestmentForm({ poles, onCancel, onAdd }: { poles: ExpensePole[]; onCancel: () => void; onAdd: (d: { label: string; pole: string; amount: string; notes: string }) => void }) {
  ...
  return (
    <div className="border border-primary/30 rounded-xl p-3 space-y-2 bg-nv-card mb-1.5">
      <input className={inp} placeholder="Ex: 2 caméras, formation ads…" value={label} onChange={e => setLabel(e.target.value)} autoFocus />
      ...
      <div className="flex gap-2">
        ...
        <button onClick={() => { if (!label.trim() || !amount) { toast.error('Libellé et montant requis'); return } onAdd({ label, pole, amount, notes: '' }) }} className="px-3 rounded-lg bg-primary text-nv-black text-xs font-medium flex items-center gap-1"><Target size={13} /> Ajouter</button>
      </div>
    </div>
  )
}
