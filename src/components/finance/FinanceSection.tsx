...
export async function FinanceSection({ previsionnel, forecastByMonth = {} }: { previsionnel: React.ReactNode; forecastByMonth?: Record<string, ForecastMonthLite> }) {
  const now = new Date()
  ...
  return (
    <FinanceHub
      ...
      investments={(investments as any[]).map(i => ({ id: i.id, month: i.month, label: i.label, pole: i.pole, amount: i.amount, done: i.done, recurring: i.recurring ?? false, notes: i.notes }))}
